...
# Class for U Net with 2 blocks
class Image_Segmentation_U2:

    def __init__(self):
        # Setup training dataset
        dataset = RD.RetinaDataset(
        ...
        self.criterion = CombinedLoss(dice_weight=0.7, focal_weight=0.3)
        # Optimizer settings
    ...
    def setup_model(self):
        ...
        # UNet with 2 layer architecture
        class UNet2Blocks(nn.Module):
            ...
            def forward(self, x):
                ...
                u2 = self.up2(b); c2 = torch.cat([u2, e2], dim=1); d2 = self.decoder2(c2)
        ...
        model = UNet2Blocks(in_channels=3, base_filters=64, out_classes=1)
        return model
    ...
    def test(self):
        ...
        print("--- Starting Testing of unet2 ---")
        self.model.eval() # Set to evaluation mode
        total_iou, total_dice, total_loss, total_acc, n_batches = 0.0, 0.0, 0.0, 0.0, 0
        with torch.no_grad():
    ...
    def save_model(self, path="U2_net.pth"):
...
def execute_all(epochs=10):
    ...
    helper = Image_Segmentation_U2()
    helper.train(epochs=epochs)
    helper.test()
    helper.save_model()

if __name__ == "__main__":
    execute_all(epochs=10)